...
M.config = {
  keymaps = {
    ...
    move_next_preorder_same_type_different_line = '<M-j>',
    move_prev_preorder_same_type_different_line = '<M-k>',
    move_up_parent = '<M-h>',
  },
  highlight = {
    enabled = false,
...
local function highlight_nodes_with_auto_clear(nodes, function_name)
  ...
  if M.config.highlight.clear_delay_ms > 0 then
    ...
    M.highlight_timer:start(
      M.config.highlight.clear_delay_ms,
      0,
...
function M.setup(opts)
  ...
  -- Set up keymaps
  if M.config.keymaps.move_next_preorder ~= nil then
  ...
  if M.config.keymaps.move_prev_preorder ~= nil then
  ...
  if M.config.keymaps.move_next_levelorder ~= nil then
  ...
  if M.config.keymaps.move_prev_levelorder ~= nil then
  ...
  if M.config.keymaps.move_next_preorder_same_type ~= nil then
    vim.keymap.set(
      'n',
      M.config.keymaps.move_next_preorder_same_type,
      M.move_next_preorder_same_type,
      { desc = 'Lemur: Move to next node of same type in pre-order traversal' }
    )
  end
  if M.config.keymaps.move_prev_preorder_same_type ~= nil then
    vim.keymap.set(
      'n',
      M.config.keymaps.move_prev_preorder_same_type,
      M.move_prev_preorder_same_type,
      { desc = 'Lemur: Move to previous node of same type in pre-order traversal' }
    )
  end
  if M.config.keymaps.move_next_preorder_different_line ~= nil then
    vim.keymap.set(
      'n',
      M.config.keymaps.move_next_preorder_different_line,
      M.move_next_preorder_different_line,
      { desc = 'Lemur: Move to next node on different line in pre-order traversal' }
    )
  end
  if M.config.keymaps.move_prev_preorder_different_line ~= nil then
    vim.keymap.set(
      'n',
      M.config.keymaps.move_prev_preorder_different_line,
      M.move_prev_preorder_different_line,
      { desc = 'Lemur: Move to previous node on different line in pre-order traversal' }
    )
  end
  if M.config.keymaps.move_next_preorder_same_type_different_line ~= nil then
    vim.keymap.set(
      'n',
      M.config.keymaps.move_next_preorder_same_type_different_line,
      M.move_next_preorder_same_type_different_line,
      { desc = 'Lemur: Move to next node of same type on different line in pre-order traversal' }
    )
  end
  if M.config.keymaps.move_prev_preorder_same_type_different_line ~= nil then
    vim.keymap.set(
      'n',
      M.config.keymaps.move_prev_preorder_same_type_different_line,
      M.move_prev_preorder_same_type_different_line,
      { desc = 'Lemur: Move to previous node of same type on different line in pre-order traversal' }
    )
  end
  if M.config.keymaps.move_next_inorder ~= nil then
  ...
  if M.config.keymaps.move_prev_inorder ~= nil then
  ...
  if M.config.keymaps.move_next_postorder ~= nil then
  ...
  if M.config.keymaps.move_prev_postorder ~= nil then
  ...
  if M.config.keymaps.move_up_parent ~= nil then
    vim.keymap.set('n', M.config.keymaps.move_up_parent, M.move_up_parent, { desc = 'Lemur: Move up through parent nodes until position changes' })
  end
...
local function get_reachable_nodes(function_name)
  ...
  if
    function_name == 'move_next_preorder'
    or function_name == 'move_prev_preorder'
    or function_name == 'move_next_preorder_different_line'
    or function_name == 'move_prev_preorder_different_line'
  then
  ...
  elseif
    function_name == 'move_next_preorder_same_type'
    or function_name == 'move_prev_preorder_same_type'
    or function_name == 'move_next_preorder_same_type_different_line'
    or function_name == 'move_prev_preorder_same_type_different_line'
  then
...
function M.move_next_preorder_different_line()
  ...
  while attempts < max_attempts do
    ...
    if next_start_row ~= initial_row then
      local target_info = get_node_info(next_node)
      log_action(
        'move_next_preorder_different_line',
        'found node on different line after ' .. (attempts + 1) .. ' attempts',
...
function M.move_prev_preorder_different_line()
  ...
  while attempts < max_attempts do
    ...
    if prev_start_row ~= initial_row then
      local target_info = get_node_info(prev_node)
      log_action(
        'move_prev_preorder_different_line',
        'found node on different line after ' .. (attempts + 1) .. ' attempts',
        current_info .. ' -> ' .. target_info
      )
      set_cursor_to_node(prev_node)
...
function M.move_prev_preorder_same_type_different_line()
  ...
  for i, same_node in ipairs(same_type_nodes) do
    if nodes_equal(same_node, node) then
      ...
      log_action(
        'move_prev_preorder_same_type_different_line',
        'no previous same type node on different line',
        current_info .. ' (type: ' .. target_type .. ')'
      )
      return
    end
  end
...
function M.move_up_parent()
  local node = get_cursor_node()
  if not node then
    log_action('move_up_parent', 'no node found at cursor', nil)
    return
  end
  ...
  while attempts < max_attempts do
    ...
    if not parent_node then
      log_action('move_up_parent', 'no parent node found', current_info)
      return
    end

    local parent_start_row, parent_start_col = parent_node:start()

    -- If the parent node starts at a different position, move there
    if parent_start_row ~= initial_row or parent_start_col ~= initial_col then
      local target_info = get_node_info(parent_node)
      log_action('move_up_parent', 'found parent at different position after ' .. (attempts + 1) .. ' attempts', current_info .. ' -> ' .. target_info)
      set_cursor_to_node(parent_node)
      return
    end
  ...
  log_action('move_up_parent', 'reached max attempts without finding different position', current_info)
end
